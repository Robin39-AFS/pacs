...
document.getElementById('patient-upload-form').addEventListener('submit', async e => {
  ...
  // Updated validation to accept multiple image formats
  if (dicomFile && !['.jpg', '.jpeg', '.png', '.gif', '.dcm'].some(ext => dicomFile.name.toLowerCase().endsWith(ext))) {
    msgEl.textContent = '❌ Please upload an image or DICOM file.';
  ...
  try {
    ...
    uploadData.append('dicom_file', dicomFile);
